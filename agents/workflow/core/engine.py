"""
agents/workflow/core/engine.py
...
class WorkflowEngine:
    """Orchestration engine for the workflow agent."""

    def __init__(self, agent_config: dict | None = None):
        self._config = agent_config or {}
        self._llm    = get_llm()
        self._tracer = get_tracer("workflow_agent")
        logger.info(f"[WorkflowEngine] initialised  provider={type(self._llm).__name__}")
