"""
agents/pdf_ingestor/workflows/nodes/clean_text_node.py
========================================================
Node function: ``clean_text_node``
...
def clean_text_node(state):
    t0=time.monotonic()
    cleaned=[{"page":p["page"],"text":p["text"].strip()} for p in state.get("extracted_pages",[])]
    return {"cleaned_pages":cleaned,"current_node":"clean_text_node",
            "execution_trace":[build_trace_entry("clean_text_node",int((time.monotonic()-t0)*1000))]}
