"""
agents/api_query/workflows/nodes/load_api_spec_node.py
========================================================
Node function: ``load_api_spec_node``
...
def load_api_spec_node(state):
    ...
    spec=client.get_spec(state.get("config",{}).get("api",{}).get("spec_url"))
    catalogue=[{"path":p,"methods":list(m.keys())} for p,m in spec.get("paths",{}).items()]
    return {"api_spec":spec,"endpoint_catalogue":catalogue,"status":ExecutionStatus.RUNNING,
            "current_node":"load_api_spec_node",
            "execution_trace":[build_trace_entry("load_api_spec_node",int((time.monotonic()-t0)*1000))]}
