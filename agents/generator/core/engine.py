"""
agents/generator/core/engine.py
...
class GeneratorEngine:
    """Orchestration engine for the generator agent."""

    def __init__(self, agent_config: dict | None = None):
        self._config = agent_config or {}
        self._llm    = get_llm()
        self._tracer = get_tracer("generator_agent")
        logger.info(f"[GeneratorEngine] initialised  provider={type(self._llm).__name__}")
