"""
agents/salesforce/core/engine.py
...
class SalesforceEngine:
    """Orchestration engine for the salesforce agent."""

    def __init__(self, agent_config: dict | None = None):
        self._config = agent_config or {}
        self._llm    = get_llm()
        self._tracer = get_tracer("salesforce_agent")
        logger.info(f"[SalesforceEngine] initialised  provider={type(self._llm).__name__}")

    def run(self, raw_input: str, session_id: str | None = None) -> dict:
        from agents.salesforce.graph import run_salesforce_agent
        return run_salesforce_agent(
            raw_input=raw_input,
