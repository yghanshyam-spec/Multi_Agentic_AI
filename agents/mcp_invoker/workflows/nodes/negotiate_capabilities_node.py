"""
agents/mcp_invoker/workflows/nodes/negotiate_capabilities_node.py
===================================================================
Node function: ``negotiate_capabilities_node``
...
def negotiate_capabilities_node(state):
    ...
    reg=state.get("mcp_registry",[])
    server_id=reg[0]["server_id"] if reg else "default"
    caps=client.negotiate(server_id)
    return {"mcp_capabilities":caps,"selected_server_id":server_id,"current_node":"negotiate_capabilities_node",
            "execution_trace":[build_trace_entry("negotiate_capabilities_node",int((time.monotonic()-t0)*1000))]}
