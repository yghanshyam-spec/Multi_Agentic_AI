"""
agents/mcp_invoker/core/engine.py
...
class McpInvokerEngine:
    """Orchestration engine for the mcp_invoker agent."""

    def __init__(self, agent_config: dict | None = None):
        self._config = agent_config or {}
        self._llm    = get_llm()
        self._tracer = get_tracer("mcp_invoker_agent")
        logger.info(f"[McpInvokerEngine] initialised  provider={type(self._llm).__name__}")

    def run(self, raw_input: str, session_id: str | None = None) -> dict:
        from agents.mcp_invoker.graph import run_mcp_invoker_agent
        return run_mcp_invoker_agent(
            raw_input=raw_input,
