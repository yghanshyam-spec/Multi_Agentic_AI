"""
agents/translation/core/engine.py
...
class TranslationEngine:
    """Orchestration engine for the translation agent."""

    def __init__(self, agent_config: dict | None = None):
        self._config = agent_config or {}
        self._llm    = get_llm()
        self._tracer = get_tracer("translation_agent")
        logger.info(f"[TranslationEngine] initialised  provider={type(self._llm).__name__}")

    def run(self, raw_input: str, session_id: str | None = None) -> dict:
        from agents.translation.graph import run_translation_agent
        return run_translation_agent(
            raw_input=raw_input,
