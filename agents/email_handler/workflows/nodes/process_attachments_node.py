"""
agents/email_handler/workflows/nodes/process_attachments_node.py
==================================================================
Node function: ``process_attachments_node``
...
def process_attachments_node(state: dict) -> dict:
    t0 = time.monotonic()
    attachments = state.get("email_attachments", [])
    processed = []
    for att in attachments:
        att_type = att.get("type", "")
        route = "PDF_INGESTOR" if "pdf" in att_type else (
            "DATA_EXTRACTOR" if "xlsx" in att_type else "OCR_PIPELINE"
        )
        processed.append({**att, "routed_to": route, "status": "QUEUED"})
    return {
        "processed_attachments": processed,
        "current_node": "process_attachments_node",
        "execution_trace": [build_trace_entry("process_attachments_node", int((time.monotonic() - t0) * 1000))],
    }
