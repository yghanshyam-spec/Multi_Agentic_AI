"""
agents/email_handler/workflows/nodes/fetch_email_node.py
==========================================================
Node function: ``fetch_email_node``

Single-responsibility node — part of the email_handler LangGraph workflow.
"""
from __future__ import annotations
import time, json, re, hashlib
from typing import Any, Optional

from shared.common import (
    get_llm, call_llm, get_prompt, log_llm_call, get_tracer,
    make_audit_event, build_trace_entry, make_base_state, new_id, utc_now,
    ExecutionStatus, build_agent_response, AgentType, safe_get, truncate_text,

    get_last_token_usage,
)
from agents.email_handler.prompts.defaults import get_default_prompt
from agents.email_handler.tools.mailbox_connector import MailboxConnector

# ── Module-level constants ────────────────────────────────────────────────────
_AGENT = "email_handler_agent"

# ── Tool instances ────────────────────────────────────────────────────────────
# (none)

# ── Private helpers ────────────────────────────────────────────────────────────
def _p(key: str, state: dict, **kw) -> str:
    override = state.get("config", {}).get("prompts", {}).get(key)
    fallback = override or get_default_prompt(f"email_handler_{key}")
    return get_prompt(f"email_handler_{key}", agent_name=_AGENT, fallback=fallback, **kw)

# ── Prompt resolver ───────────────────────────────────────────────────────────
def _p(key, state, **kw):
    fb = state.get("config", {}).get("prompts", {}).get(key) or get_default_prompt(f"email_handler_{key}")
    return get_prompt(f"email_handler_{key}", agent_name="email_handler", fallback=fb, **kw)


def fetch_email_node(state: dict) -> dict:
    t0 = time.monotonic()
    connector = MailboxConnector(state.get("config", {}).get("mailbox", {}))
    emails = connector.fetch_unread(max_count=state.get("config", {}).get("batch_size", 5))
    raw_email = emails[0] if emails else {
        "id": "mock-001", "subject": state.get("raw_input", ""),
        "sender": "unknown@example.com", "body": state.get("raw_input", ""),
        "attachments": [], "headers": {}, "received_at": "",
    }
    return {
        "email_id": raw_email.get("id"),
        "raw_email": raw_email,
        "email_subject": raw_email.get("subject", ""),
        "email_sender": raw_email.get("sender", ""),
        "email_body": raw_email.get("body", ""),
        "email_attachments": raw_email.get("attachments", []),
        "status": ExecutionStatus.RUNNING,
        "current_node": "fetch_email_node",
        "execution_trace": [build_trace_entry("fetch_email_node", int((time.monotonic() - t0) * 1000))],
        "audit_events": [make_audit_event(state, "fetch_email_node", f"EMAIL_FETCHED:{raw_email.get('id')}")],
    }
