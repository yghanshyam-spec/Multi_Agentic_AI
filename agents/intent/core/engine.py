...
class IntentEngine:
    """Orchestration engine for the intent agent."""

    def __init__(self, agent_config: dict | None = None):
        self._config = agent_config or {}
        self._llm    = get_llm()
        self._tracer = get_tracer("intent_agent")
        logger.info(f"[IntentEngine] initialised  provider={type(self._llm).__name__}")
