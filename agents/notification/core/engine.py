"""
agents/notification/core/engine.py
...
class NotificationEngine:
    """Orchestration engine for the notification agent."""

    def __init__(self, agent_config: dict | None = None):
        self._config = agent_config or {}
        self._llm    = get_llm()
        self._tracer = get_tracer("notification_agent")
        logger.info(f"[NotificationEngine] initialised  provider={type(self._llm).__name__}")

    def run(self, raw_input: str, session_id: str | None = None) -> dict:
        from agents.notification.graph import run_notification_agent
        return run_notification_agent(
            raw_input=raw_input,
