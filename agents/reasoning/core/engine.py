"""
agents/reasoning/core/engine.py
...
class ReasoningEngine:
    """Orchestration engine for the reasoning agent."""

    def __init__(self, agent_config: dict | None = None):
        self._config = agent_config or {}
        self._llm    = get_llm()
        self._tracer = get_tracer("reasoning_agent")
        logger.info(f"[ReasoningEngine] initialised  provider={type(self._llm).__name__}")
