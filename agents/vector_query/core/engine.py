"""
agents/vector_query/core/engine.py
...
class VectorQueryEngine:
    """Orchestration engine for the vector_query agent."""

    def __init__(self, agent_config: dict | None = None):
        self._config = agent_config or {}
        self._llm    = get_llm()
        self._tracer = get_tracer("vector_query_agent")
        logger.info(f"[VectorQueryEngine] initialised  provider={type(self._llm).__name__}")

    def run(self, raw_input: str, session_id: str | None = None) -> dict:
        from agents.vector_query.graph import run_vector_query_agent
        return run_vector_query_agent(
            raw_input=raw_input,
