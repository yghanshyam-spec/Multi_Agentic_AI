"""
agents/vector_query/workflows/nodes/rerank_chunks_node.py
===========================================================
Node function: ``rerank_chunks_node``
...
def rerank_chunks_node(state):
    t0=time.monotonic()
    chunks=sorted(state.get("filtered_chunks",[]),key=lambda c:c.get("score",0),reverse=True)[:4]
    return {"reranked_chunks":chunks,"current_node":"rerank_chunks_node",
            "execution_trace":[build_trace_entry("rerank_chunks_node",int((time.monotonic()-t0)*1000))]}
