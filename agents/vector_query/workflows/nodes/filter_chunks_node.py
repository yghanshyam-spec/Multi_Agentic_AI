"""
agents/vector_query/workflows/nodes/filter_chunks_node.py
===========================================================
Node function: ``filter_chunks_node``
...
def filter_chunks_node(state):
    t0=time.monotonic()
    filters=state.get("config",{}).get("filters",{})
    chunks=state.get("retrieved_chunks",[])
    min_score=float(filters.get("min_score",0.5))
    filtered=[c for c in chunks if c.get("score",1.0)>=min_score]
    return {"filtered_chunks":filtered,"current_node":"filter_chunks_node",
            "execution_trace":[build_trace_entry("filter_chunks_node",int((time.monotonic()-t0)*1000))]}
