...
class SqlEngine:
    """Orchestration engine for the sql agent."""

    def __init__(self, agent_config: dict | None = None):
        self._config = agent_config or {}
        self._llm    = get_llm()
        self._tracer = get_tracer("sql_agent")
        logger.info(f"[SqlEngine] initialised  provider={type(self._llm).__name__}")
